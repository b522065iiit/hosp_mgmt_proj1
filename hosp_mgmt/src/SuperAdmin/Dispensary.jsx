...
export default function Dispensary() {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await fetch('http://localhost:3000/disp_stock', {
        method: 'POST',
        body: JSON.stringify({ 'send': true, 'desg': 'dispensary' }),
        headers: {
  ...
  return (
    <div className="flex flex-col space-y-5">
      <p>Current Dispensary Incharge: {office.name} (ID: {office.emp_id})</p>
      <h2 className="text-center text-red-700 font-Nunito font-bold">Medicine Stock available at Dispensary</h2>
